...
#include "../GiftLink/Blockchain.h"
#include "../GiftLink/BroadcastListener.h"
using namespace std;
...
int main()
{
	const uint8_t myPrivateKey[SHA256_DIGEST_VALUELEN] = { 0x00, 0xd2, 0x26, 0x4b, 0x25, 0xbe, 0xd8, 0x25, 0xd0, 0x96, 0x42, 0x1b,
	...
	const uint8_t administratorPrivateKey[SHA256_DIGEST_VALUELEN] = { 0x67, 0xd2, 0x26, 0x4b, 0x25, 0xbe, 0xd8, 0x25, 0xd0, 0x96, 0x42, 0x1b,
		 0x52, 0x13, 0x0d, 0x26, 0x2c, 0x26, 0x41, 0xa0, 0x7f, 0x06, 0xb0, 0xff, 0x33, 0x75, 0x9f, 0x06, 0x89, 0x96, 0xd2, 0x1a };
	Wallet administratorWallet(administratorPrivateKey);
	cout << "Administrator wallet was created\n\n";
...
USE: // 유가증권 사용
	{
		...
		// 코드 스캔
		const uint8_t * recipientPublicKeyHash = administratorWallet.getPublicKeyHash();					// 받는 사람 입력 또는 코드 스캔
...
SEND: // 유가증권, 코인 전송
	{
		...
		// 코드 스캔
		const uint8_t * recipientPublicKeyHash = administratorWallet.getPublicKeyHash();					// 받는 사람
